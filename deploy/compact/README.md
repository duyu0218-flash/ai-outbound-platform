...
API 第一阶段启用每进程最多 8 个等待请求、最长 50ms 的短队列，Compose 显式设置 `REQUEST_ADMISSION_MAX_WAITERS=8` 和 `REQUEST_ADMISSION_TIMEOUT_SEC=0.05`。保持每进程回调 4、普通请求 2、总处理 6 及连接池 6 不变；等待期间不进入认证或数据库处理，等待名额耗尽或超时仍返回带 `Retry-After` 的 503。该等待预算由请求类别共享，尚非独立 FIFO 队列；它用于吸收突发，不代表持续吞吐提高。回滚时同时还原 Compose 的等待上限为 0，避免仅修改 env 文件被 Compose 覆盖。实际多进程结果见[短队列验收记录](../../docs/reviews/20260907-admission-step1.md)。

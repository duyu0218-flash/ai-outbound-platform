...
- 依赖锁定为 `pipecat-ai[openai,websocket]==1.8.1+outbound.1`，并与 `compatibility-matrix.toml` 和 `PIPECAT_VERSION` 一致。
- 安装方式见[受控补丁包](pipecat-controlled-patch.md)：基于官方 1.8.1 移除 NLTK，校验上游和产物摘要后从本地 wheelhouse 安装。

...
此外，生产配置必须关闭应用启动迁移（`AUTO_MIGRATE=false`），设置允许拨打号段和租户日拨号上限，并使用签名Webhook。应用容器以非root、只读根文件系统、无额外Linux capabilities运行；CI生成SBOM并执行依赖漏洞审计。上述代码门禁不能替代数据库高可用、异地备份、运营商原始录音删除、MFA/企业SSO和许可证法务复核。

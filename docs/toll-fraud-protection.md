...
| 入口 | 本轮验证方法 | 边界 |
| --- | --- | --- |
| 登录 / 退出 / 账号锁定 | 后端 TestClient 回归；浏览器实际登录、退出返回登录页 | 浏览器账号锁定及错误提示完整性未验证 |
| 线路新增 / 修改 / 停用 | API 保存、回显、权限拒绝、审计回归 | 仅合成配置 |
| 合规设置 / 并发容量 | API 保存回显、独立凭据、越上限拒绝；浏览器将隔离测试库并发从 20 降为 5，保存及离开后返回回显一致 | 浏览器合规表单与生产独立凭据流程未验证 |
...
| 页面入口 | 隔离实例 `/admin` 登录、仪表盘、系统配置、外呼线路入口及返回路径实际点击通过 | 非全站页面验收；未触发真实外呼 |

外呼项目不包含“首页→搜索→门店/房型→购物车→支付”购物链路；此项不适用。对应业务回归是登录→客户/任务→呼叫→媒体/转人工→结束记录→退出。

页面检查使用单独 SQLite 测试库、mock 电话服务并关闭调度器，没有修改现有业务实例。最初访问 `/` 返回错误，该根路径没有页面；改用实际管理端入口 `/admin` 后可正常访问。错误密码提交后仍停留登录页，但本轮未捕获到可确认的错误提示，不能把这项列为页面验收通过。完整外呼链路由上述后端及媒体回归覆盖，不等同于完整浏览器业务链路已通过。

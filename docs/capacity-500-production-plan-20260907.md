# 自有外呼平台替代三方系统：500 并发上线技术方案

> 已由 [单节点 200 路、集群 500 路修订版](capacity-500-node200-plan-20260907.md) 替代。本文保留为旧版方案，节点容量和服务器数量不再作为当前规划。

日期：2026-09-07。基于本项目当前源码，以及本轮 P1/P2 修复后的隔离压测。
...
| 媒体故障余量 | 5 个独立媒体节点，每节点须实测通过 150 路 | 合计 750 路节点能力，业务上仍只开放 500；少一个节点后剩余能力 600 路 |
...
- **通话与媒体：** 五个独立宿主节点，各部署配对的 FreeSWITCH、voice gateway 和 Pipecat；统一分配、固定归属、独立录音与事件缓冲。集群资格测试须把编解码、重采样、AI 音频桥、双声道录音和人工桥接一起打开。
...
### 5. 媒体和 AI 服务：五节点固定归属，预热而不是临时起进程

每个媒体节点启动常驻进程和连接池，分配 150 路已验收容量；正常 500 路分散为约每节点 100 路。升级时先 draining，不再接新呼叫，等待本节点通话结束后再停止进程。
...
| FreeSWITCH＋网关＋Pipecat | 5 个独立节点，各 16 vCPU / 32GB | 单节点先通过 150 路真实媒体；正常约 100 路/节点；不得把 5 个容器部署在同一故障宿主机上冒充冗余 |
...
| Redis | 托管高可用或独立主从与仲裁，单节点 4 vCPU / 8GB 起测 | 用于缓存、通知和辅助协调；真实占用以持久账本为准 |
| 存储与入口 | 可靠对象存储、双入口/负载均衡；可部署 2 个 PgBouncer 实例 | 数据库连接预算跨全部实例求和；录音与日志不争用媒体系统盘 |

上述每个媒体节点的 150 路资格包含自有 FreeSWITCH、音频桥和 Pipecat 的整体表现；如三者拆机部署，每一层都要满足同样的并发和故障余量，不能沿用合并部署的机器总数。不同可用区/机房整体故障不在“少 1 个节点”承诺范围内，需另行增加冗余预算。
...
| 阶段 | 测试方式 | 必须通过的结果 |
|---|---|---|
| 单节点资格 | 一个媒体节点，真实音频＋ASR/TTS/LLM，50→100→150 路 | 150 路连续 2 小时，达到对话与打断时延；CPU/内存/音频丢帧有余量 |
...
**故障保证的边界：** 5×150 的预算保证的是少一个媒体节点后还有足够的承载资源，不保证故障节点上的现有音频会话零中断。现有状态和音频架构不支持任意媒体会话透明热迁移。若要求“任意媒体节点宕机仍500通全部不断话”，必须追加可热接管的媒体底座、状态同步与专门故障验收，不能用 N+1 资源数字作零掉话承诺。
...
4. **媒体节点资格与集群联调。** 单节点150路通过后扩到五节点，联通真实ASR/TTS/LLM并完成500路阶梯、长稳和故障测试。
5. **替代交付与上线发布。** 完成第六节业务闭环、历史数据交接与对账，备份后部署兼容版本；按小规模→250→500灰度切换，确保旧平台对应任务停止、同一名单只有自有系统调度。任何额定指标不达标则回退可用容量，不把500写成已完成。

本文是技术方案交付；本轮没有执行上述扩容改造、采购或生产部署。现有证据仍为100路模拟拨号、100 RPS短时全成功回调；500路端到端上线能力尚未验证。运营商线路500并发沿用用户已确认事实，不重复验证其是否足额。

#!/usr/bin/env python3
"""Two real async AI processes and 256 delayed synthetic model requests.
...
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/'artifacts/node200'
...
env=dict(os.environ,ENV='test',DATABASE_URL=DSN,DATABASE_URL_API=DSN,DATABASE_URL_BOOTSTRAP=DSN,
 REDIS_URL='redis://127.0.0.1:16443/4',TASK_WORKER_ROLE='ai',TASK_AI_CONCURRENCY='128',
 AI_DB_THREADS='2',AI_ACTION_THREADS='2',DATABASE_POOL_SIZE='3',DATABASE_MAX_OVERFLOW='0',
...
async def main():
    ...
    async with httpx.AsyncClient(trust_env=False) as http:
        ...
        workers=[launch([sys.executable,'-m','app.ai_worker'],ROOT/'backend',{'AI_WORKER_HEALTH_PATH':str(OUT/f'ai-health-{i}.json')}) for i in range(2)]
        ...
        try:
            for _ in range(600):
                ...
                if states.get('completed')==256:break
                if any(p.poll() is not None for p in workers):raise RuntimeError('AI worker exited; inspect logs')
                await asyncio.sleep(.1)
            assert stats['peak']==256 and states.get('completed')==256,(stats,states)
            with session_scope() as s:
                attempts=list(s.exec(select(TaskOutbox.attempts)).all())
            assert max(attempts)==1
            result={'ai_worker_processes':2,'slots_per_process':128,'model_peak':stats['peak'],
              ...
              'db_threads_per_process':2,'action_threads_per_process':2,'pool_connections_per_process':3,
              'db_while_256_models_wait':[s for s in dbstates if s['model_active']==256][:5],

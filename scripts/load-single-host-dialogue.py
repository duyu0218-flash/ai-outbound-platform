...
from app.models import CallSession,CallStatus,CallMode,TaskOutbox,TaskState,CallMetric,SpeechTurn
...
async def main():
    ...
    async with httpx.AsyncClient(trust_env=False,timeout=10,limits=httpx.Limits(max_connections=100)) as http:
        for _ in range(100):
            try:
                ready=(await http.get('http://127.0.0.1:18900/readyz')).status_code==200 and (await http.get('http://127.0.0.1:18941/stats')).status_code==200
        ...
        cfg=VoiceSettings(_env_file=None,voice_security_db_path=str(DEST/'callback-ledger.db'),
        ...
        with session_scope() as s:
            turns=s.exec(select(func.count()).select_from(SpeechTurn).where(SpeechTurn.is_final.is_(True))).one()
        ...
        result=dict(synthetic_active_calls=500,final_transcripts_per_second=RATE,media_events_per_second=RATE*2,duration_seconds=30,
            ...
            correctness_passed=queue['pending_callbacks']==0 and statuses['200']==TOTAL*3 and states.get('completed')==TOTAL and turns==TOTAL)
